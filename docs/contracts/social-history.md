...
El paquete publicado `1.25.24` incluye un formulario AMPATH independiente para
historia social, preparado inicialmente como `1.25.23` en el PR #235.
...
El UUID del Form se deriva del nombre y versión con la regla de Initializer
`2.13.0-sihsalus.1`, verificada por el
[validador del contrato](../../.github/scripts/validate_social_history_contract.py)
con el cálculo compartido del [validador AMPATH](../../.github/scripts/validate_ampath_forms.py).
El UUID del JSON no sirve como identificador REST del Form.
Cambiar la versión requiere coordinar el nuevo
...
Se debe cargar una versión que incluya este formulario antes de habilitar el
frontend dependiente. La publicación del paquete no modifica el pin del distro
ni acredita un despliegue. La aceptación en DEV/QLTY requiere verificar
importación con Initializer, UUID REST efectivo,
...
Completar con las [comprobaciones comunes](../development.md#validación-local),
incluidas la identidad AMPATH y la integridad de conceptos.
